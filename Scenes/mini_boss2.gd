...
func take_damage():
	...
	if health <= 0:
		spawn_health_pickup()  # Spawn the heart pickup
		health_depleted.emit()
		queue_free()

func spawn_health_pickup():
	var health_pickup = HealthPickupScene.instantiate()
	get_parent().add_child(health_pickup)
	health_pickup.global_position = global_position  # Drop the heart at the enemy's position
